...
const statusOptions: AdminStatusFilter[] = [
  'all',
  'verified',
  'active',
  'new',
...
export function AdminDashboard() {
  const activeTab = useAdminUiStore((state) => state.activeTab)
  ...
  const visibleModeration = useMemo(() => {
    const keyword = normalize(search)
    return adminModerationItems.filter((item) => {
      const matchesStatus = status === 'all' || item.status === status
      const matchesSearch =
        !keyword ||
        [item.title, item.owner, item.note].some((value) =>
          normalize(value).includes(keyword),
        )
      return matchesStatus && matchesSearch
    })
        ...
        <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-500 md:text-base">
              Theo dõi người dùng, kiểm duyệt tin đăng và xử lý các cảnh báo vận hành.
            </p>
          </div>
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_180px] lg:w-[520px]">
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder="Tìm theo tên, email, tin đăng..."
                />
              </div>
              <select
                    ...
                    <tr key={user.id}>
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-3">
                          <Avatar name={user.name} className="bg-slate-200 text-slate-700" />
                          <div>
                      ...
                      <td className="px-4 py-4">
                        <span className={cn('rounded-lg px-2.5 py-1 text-xs font-bold', adminStatusStyles[user.status])}>
            ...
            <div className="grid gap-3 p-4 md:grid-cols-3">
              {visibleModeration.map((item) => (
                ...
                >
                  ...
                  <span className={cn('mt-4 inline-flex rounded-lg px-2.5 py-1 text-xs font-bold', adminStatusStyles[item.status])}>
